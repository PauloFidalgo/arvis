...
module cv32e40p_hwloop_regs #(
    parameter HW_LOOP     = 2,
    parameter N_REG_BITS = $clog2(HW_LOOP)
) (
    input logic clk,
    input logic rst_n,
    ...
    input logic [HW_LOOP-1:0] hwlp_regid_i,  // selects the register set
  ...
  logic [HW_LOOP-1:0][31:0] hwlp_counter_q, hwlp_counter_n;
  ...
  always_ff @(posedge clk, negedge rst_n) begin : HWLOOP_REGS_COUNTER
    if (rst_n == 1'b0) begin
      ...
      for (i = 0; i < HW_LOOP; i++) begin
        if ((hwlp_we_i[2] == 1'b1) && (i[HW_LOOP-1:0] == hwlp_regid_i)) begin
