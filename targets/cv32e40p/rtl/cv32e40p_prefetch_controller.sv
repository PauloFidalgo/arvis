...
module cv32e40p_prefetch_controller #(
    ...
    parameter HW_LOOP = 1,  // PULP ISA Extension (including PULP specific CSRs and hardware loop, excluding cv.elw)
    ...
    // ARVIS_HWLP_BEGIN: prefetch_ports
    // ARVIS_HWLP_END: prefetch_ports
  ...
  // ARVIS_HWLP_BEGIN: prefetch_proactive_wrap
  // ARVIS_HWLP_END: prefetch_proactive_wrap

  // ARVIS_HWLP_BEGIN: prefetch_fifo_cnt_masked
  assign fifo_cnt_masked = (branch_i || hwlp_jump_i) ? '0 : fifo_cnt_i;
  // ARVIS_HWLP_END: prefetch_fifo_cnt_masked
    ...
    case (state_q)
      ...
      IDLE: begin
        begin
          ...
          // ARVIS_HWLP_BEGIN: REMOVE
          else if (hwlp_jump_i) begin
            trans_addr_o = hwlp_target_i;
          end 
          // ARVIS_HWLP_END: REMOVE
          else begin
            // ARVIS_HWLP_BEGIN: prefetch_idle_addr
            trans_addr_o = trans_addr_incr;
            // ARVIS_HWLP_END: prefetch_idle_addr
          end
        end
        // ARVIS_HWLP_BEGIN: prefetch_idle_case_if
        if ((branch_i || hwlp_jump_i) && !(trans_valid_o && trans_ready_i)) begin
          // Taken branch, but transaction not yet accepted by bus interface adapter.
          next_state = BRANCH_WAIT;
        end
        // ARVIS_HWLP_END: prefetch_idle_case_if
      end  // case: IDLE
  ...
  // ARVIS_HWLP_BEGIN: prefetch_gen_hwlp
  generate
  ...
  endgenerate
  // ARVIS_HWLP_END: prefetch_gen_hwlp
  //////////////////////////////////////////////////////////////////////////////
  ...
  always_comb begin
    ...
    // be flushed there)
    // ARVIS_HWLP_BEGIN: prefetch_controller_flush_chain
    ...
    else if (resp_valid_i && (flush_cnt_q > 0)) begin
      next_flush_cnt = flush_cnt_q - 1'b1;
    end
    // ARVIS_HWLP_END: prefetch_controller_flush_chain
  end
  ...
  always_ff @(posedge clk, negedge rst_n) begin
    if (rst_n == 1'b0) begin
      ...
      // ARVIS_HWLP_BEGIN: prefetch_controller_registers_if
      if (branch_i || hwlp_jump_i || (trans_valid_o && trans_ready_i)) begin
        trans_addr_q <= trans_addr_o;
      end
      // ARVIS_HWLP_END: prefetch_controller_registers_if
    end
  end
