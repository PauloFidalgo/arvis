...
| **6.7** | this | ``RISCVGCCToolchain`` in ``toolchains/riscv_gcc.py`` (compile/assemble/disassemble via GCC+binutils); ``BenchmarkWorkload.hex_for_variant()`` resolves variant-specific hex paths; ``Pipeline.run_full_verification()`` drives all 5 standard variants through the unified path.  15 integration tests in ``test_pipeline_e2e.py``. |
| **6.7+** | this | GA / exhaustive HW_LOOP selection migrated to sweep framework: ``GeneticSearch`` (binary GA in ``core/sweep.py``), ``LoopSelectionSweep`` (in ``strategies/sweep/loop_selection.py``), ``LoopSelectionEvaluator`` (in ``targets/cv32e40p/sweep_evaluators.py``).  ``_loop_selection_via_sweep`` gateway in ``pipeline/runner.py``. |
| **7** | this | Top-level verification gateway: ``run_pipeline`` dispatches to ``_run_verification_via_pipeline`` when ``--use-pipeline-runner`` is set, routing the 5-variant emission through ``Pipeline.run_full_verification``.  ``TargetCore.standard_variants`` makes the variant set a target property (no hard-coded cv32e40p import in the pipeline).  ``ctx.pipeline_result`` exposes the result to downstream consumers. |
